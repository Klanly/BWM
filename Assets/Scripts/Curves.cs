...
{
	public static Vector3 Interp(Path pts, float t, Easing.EaseType easeType = Easing.EaseType.linear)
	{
	...
	public static Vector3 MoveOnPath(Path pts, Vector3 currentPosition, ref float pathPosition,  float maxSpeed=1f, float smoothnessFactor=100, Easing.EaseType easeType = Easing.EaseType.linear)
	{
		maxSpeed *= Time.deltaTime;
		pathPosition = Mathf.Clamp01(pathPosition);
		var goal = Interp(pts, pathPosition, easeType);
		float distance;
		while((distance = (goal - currentPosition).magnitude) <= maxSpeed && pathPosition < 1)
		{
			...
			goal = Interp(pts, pathPosition, easeType);
		}
		if(distance != 0)
		{
	...
	public static Vector3 MoveOnPath(Path pts, Vector3 currentPosition, ref float pathPosition, ref Quaternion rotation,  float maxSpeed=1f, float smoothnessFactor=100, Easing.EaseType easeType = Easing.EaseType.linear)
	{
		var result = MoveOnPath(pts, currentPosition, ref pathPosition, maxSpeed, smoothnessFactor, easeType);
		rotation =  result.Equals(currentPosition) ? Quaternion.identity : Quaternion.LookRotation(result - currentPosition);
		return result;
	}
	
	public static Quaternion RotationBetween(Path pts, float t1, float t2, Easing.EaseType easeType = Easing.EaseType.linear)
	{
	...
	public static void GizmoDraw(Vector3[] pts, float t,Easing.EaseType easeType = Easing.EaseType.linear) {
		...
		for (int i = 1; i <= 20; i++) {
			float pm = (float) i / 20f;
			Vector3 currPt = Interp(pts, pm, easeType);
			Gizmos.DrawLine(currPt, prevPt);
			prevPt = currPt;
		}
		
		Gizmos.color = Color.blue;
		Vector3 pos = Interp(pts, t, easeType);
		Gizmos.DrawLine(pos, pos + Velocity(pts, t, easeType));
	}
